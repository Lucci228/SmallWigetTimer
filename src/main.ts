...
const min_timer = 5;
const max_timer = 10;
const togglePlay = (audio: { paused: boolean; play: () => void; pause: () => void; }) => audio.paused ? audio.play() : audio.pause();
...
function refreshTitle(): void{
  ...
  if (title_container) {
    if (timerReset)
      title_container.innerText = "Beb Moments in Progress"
    else
      title_container.innerText = "Dai mesaj lui BEB!!!!"
  }
}
